                    ...
                    <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $classarm->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $classarm->code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $classarm->created_at }}</td>

                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <a href="{{ route('classarms.edit', $classarm->id) }}" 
                                           class="inline-block px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-md hover:bg-blue-700 transition">
                                            Edit
                                        </a>
                                        
                        @if($classarm->status)
                        <form class="inline-block"  action="{{ route('classarms.deactivate', $classarm->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to deactivate this classarm?')">
                            @csrf 
                        ...
                        @else 
                         <form class="inline-block"  action="{{ route('classarms.activate', $classarm->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to activate this classarm?')">
                            @csrf 
