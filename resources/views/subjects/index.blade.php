                    ...
                    <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $subject->name ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $subject->code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $subject->created_at }}</td>

                                   <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <a class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-gray-700 transition" href="{{ route('subjects.edit', $subject->id) }}">Edit</a>
                                        @if($subject->status)
                                        <form class="inline-block"  action="{{ route('subjects.deactivate', $subject->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to deactivate this subject?')">
                                            @csrf 
                                        ...
                                        @else 
                                         <form class="inline-block" action="{{ route('subjects.activate', $subject->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to activate this subject?')">
                                            @csrf 
