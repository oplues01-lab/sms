...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-lg shadow-md p-8">
        <div class="mb-6">
            <a href="{{ route('teacher.assignments.show', $assignment) }}" class="text-blue-600 hover:text-blue-800 font-medium mb-4 inline-block">
                ← Back to Assignment
            </a>
        ...
        <form method="POST" action="{{ route('teacher.assignments.update', $assignment) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="class_arm_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($classArms as $arm)
                                <option value="{{ $arm->id }}" {{ old('class_arm_id', $assignment->class_arm_id) == $arm->id ? 'selected' : '' }}>
                                    {{ $arm->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="term_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            @foreach($terms as $term)
                                <option value="{{ $term->id }}" {{ old('term_id', $assignment->term_id) == $term->id ? 'selected' : '' }}>
                                    {{ $term->name }}
                                </option>
                            @endforeach
                        </select>
